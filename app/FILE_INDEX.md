...
| `tooltips.py` | 46 | UI copy | Stores reusable tooltip text for Tukey windows, spectrum slices, and rolling min-max envelopes. |
| `controllers/action_handler.py` | 180 | Controller | Handles multi-step actions (comparison file selection, CSV exports, Ansys template generation). |
...
| `analysis/ansys_exporter.py` | 184 | Integration | Automates Ansys Mechanical to build harmonic or transient templates from processed loads. |
...
| `ui/tab_part_loads.py` | 123 | Qt widget | Part loads tab with side filtering, time-domain conditioning, data extraction, and Ansys export triggers. |
